...
export default function TypeaheadActores(props: TypeaheadActoresProps){
    ...
    function manejarBusqueda(query: string){
        setCargando(true);
        clienteAPI.get<ActorPelicula[]>(`/actores/${query}`).then(res => {
            setActores(res.data);
            setCargando(false);
        });
    }
        ...
        <>
            ...
            <AsyncTypeahead
                ...
                onChange={(actores: Option[]) => {
                    const actorSeleccinado = actores[0] as ActorPelicula;
                    if (props.actores.findIndex(x => x.id === actorSeleccinado.id) === -1){
                        actorSeleccinado.personaje = '';
                        props.onAdd([...props.actores, actorSeleccinado]);
                    }
                }}
                options={actores}
                ...
                renderMenuItemChildren={(opcion: Option) => {
                    ...
                    return(
                        <>
                            ...
                            style={{height:'64px',width:'64px',marginRight:'1px'}} />
            ...
            <ul className="list-group">
                {props.actores.map(actor => (
                    <li draggable={true} onDragStart={() => manejarDragStart(actor)} onDragOver={()=> manejarDragOver(actor)} className="list-group-item d-flex align-items-center" key={actor.id}>
                        ...
                        <div className="flex-grow-1 mx-3">
                            <input className="form-control" type="text" placeholder="Personaje:" value={actor.personaje} onChange={e => {
